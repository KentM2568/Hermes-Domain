...
public class WallRunTutorial : MonoBehaviour
{
    ...
    //public float wallRunForwardForce;
    //public float wallRunDownwardForce;
    public float wallrunForwardMultiplier;
    ...
    private void StartWallrun()
    {
        ...
        {
            rb.AddForce(orientation.forward * wallrunForce * Time.deltaTime * wallrunForwardMultiplier);
 ...
 //   public Vector2 velocityFloat = rb.velocity.magnitude.ToFloat();
    ...
    public float jumpExtraForwardForce;
    public float jumpExtraSideForce;
    ...
    public float rocketForwardMultiplier;
    [SerializeField]
    ...
    [SerializeField]
    float rocketTimer;
    public float rocketUpwardMultiplier;
  ...
  //  public String magnitudeString = rb.velocity.magnitude.ToString();
    ...
    private void MyInput()
    {
        ...
        if (Input.GetKeyDown(KeyCode.D) && dTapTimes <= 1)
        {
            dTapTimes++;
            Invoke("ResetDTapTimes", 0.3f);
        }
        if (dTapTimes == 2 && flashesLeft > 0 && x > 0)
        { 
            SideFlash(true);
        }

        if (Input.GetKeyDown(KeyCode.A) && aTapTimes <= 1)
        {
            aTapTimes++;
            Invoke("ResetATapTimes", 0.3f);
        }
        if (aTapTimes == 2 && flashesLeft > 0 && x < 0)
        {
            SideFlash(false);
        }
    ...
    private void ResetDTapTimes()
    {
        dTapTimes = 0;
    }

    private void ResetATapTimes()
    {
        aTapTimes = 0;
    }
    ...
    private void ActivateGravity()
    {
        ...
        {
            rb.AddForce(dashStartVector * -dashForce * 0.4f);
        }
    }
   ...
   //     flashCooldown = 30;
    ...
    private void StartRocketBoost()
    {
        ...
        */
        ...
        rb.AddForce(orientation.forward * rocketForce * Time.deltaTime * 1f * rocketForwardMultiplier);
        rb.AddForce(Vector3.up * rocketForce * Time.deltaTime * 2f * rocketUpwardMultiplier);
